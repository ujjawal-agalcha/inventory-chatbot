...
from excel.master_sheet import (
...
class TestMasterSheetExport(unittest.TestCase):
    ...
    def test_03_no_stale_data_live_update(self):
        ...
        # Step 1: Set stock to 20 (> 10 -> GREEN)
        ...
        for r in range(2, ws_1.max_row + 1):
            if ws_1.cell(row=r, column=2).value == "Arduino Uno R3":
                row_num = r
                break

        self.assertIsNotNone(row_num)
        self.assertEqual(ws_1.cell(row=row_num, column=8).value, 20)
        self.assertEqual(ws_1.cell(row=row_num, column=8).fill.start_color.rgb, "FFC6EFCE")  # GREEN
        print("[PASS] Initial state: Stock = 20 -> GREEN (FFC6EFCE)")

        # Step 2: Change stock to 5 (< 10 -> RED)
        update_product_stock("Arduino Uno R3", 5)
        excel_bytes_2 = generate_master_sheet_bytes()
        wb_2 = openpyxl.load_workbook(io.BytesIO(excel_bytes_2))
        ws_2 = wb_2["Master Inventory"]

        self.assertEqual(ws_2.cell(row=row_num, column=8).value, 5)
        self.assertEqual(ws_2.cell(row=row_num, column=8).fill.start_color.rgb, "FFFFC7CE")  # RED
        print("[PASS] Dynamic update: Stock changed 20 -> 5 -> RED (FFFFC7CE). NO STALE DATA confirmed!")

        # Step 3: Change stock to 10 (= 10 -> YELLOW)
        update_product_stock("Arduino Uno R3", 10)
        excel_bytes_3 = generate_master_sheet_bytes()
        wb_3 = openpyxl.load_workbook(io.BytesIO(excel_bytes_3))
        ws_3 = wb_3["Master Inventory"]

        self.assertEqual(ws_3.cell(row=row_num, column=8).value, 10)
        self.assertEqual(ws_3.cell(row=row_num, column=8).fill.start_color.rgb, "FFFFEB9C")  # YELLOW
        print("[PASS] Dynamic update: Stock changed 5 -> 10 -> YELLOW (FFFFEB9C).")
